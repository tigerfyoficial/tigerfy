...
async function createStep({ offerId, name, duplicate = false, fromStepId = null }) {
  ...
  // copiar configs da etapa atual, se solicitado (UI atual não usa duplicar, mas mantemos compat)
...
/* Excluir etapa (protege Etapa 1) */
async function deleteStep({ offerId, stepId }) {
  // valida step e bloqueia a 1
  const { data: step, error: e0 } = await supabase
    .from("offer_steps")
    .select("id, step_no")
    .eq("id", stepId)
    .eq("offer_id", offerId)
    .single();
  if (e0) return { error: e0 };
  if (step && step.step_no === 1) return { error: new Error("cannot_delete_first_step") };
